...
def test_default_settings(unit_system: str, expected_defaults: Dict[str, Any]) -> None:
    ...
    concrete = Concrete(
        name="conc", f_c=25 * MPa if unit_system == "metric" else 3000 * psi
    )
    ...
    for key, expected_value in expected_defaults.items():
        assert (
            settings.get_setting(key) == expected_value
        ), f"Mismatch for {key} in {unit_system} system"
