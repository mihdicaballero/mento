...
def assert_quantity_equal(q1: Quantity, q2: Quantity, rtol: float = 1e-9) -> None:
    ...
    assert pytest.approx(q1.magnitude, rel=rtol) == q2.magnitude, f"Magnitudes mismatch: {q1.magnitude} vs {q2.magnitude}"
...
def test_override_specific_metric_setting() -> None:
    ...
    assert_quantity_equal(settings.stirrup_diameter_ini, BeamSettings._metric_defaults["stirrup_diameter_ini"])
...
def test_global_beam_settings_is_metric_by_default(metric_settings: BeamSettings) -> None:
