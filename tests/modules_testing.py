...
def section() -> None:
    ...
    section = Section(c_c=25*mm, concrete=concrete, steel_bar=steel_bar)


def rectangular() -> None:
    concrete = Concrete("C25", f_c=6 * ksi)
    steel_bar = SteelBar(name="ADN 420", f_y=60 * ksi)
    section = RectangularSection(concrete=concrete, steel_bar=steel_bar, c_c=25*mm, width=12 * inch, height=20 * inch
    )
